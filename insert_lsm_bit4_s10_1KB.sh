...
#    __checkOutBranch NoSeekCompaction
	...
	if [ "$requestdistribution" = "zipfian" ]; then
	    echo "zipfian"
	    __runLSM insert_bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" /home/ming/experiment/hlsm_"$DISK"_read/skipratio2_"$requestdistribution""$zipfianconst"_sizeRatio"$sizeRatio" "$level"  "$lsmtype" "$bloombits"
	else
	    echo "$requestdistribution"
	    __runLSM insert_bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" /home/ming/experiment/hlsm_"$DISK"_read/skipratio2_"$requestdistribution"_sizeRatio"$sizeRatio" "$level"  "$lsmtype" "$bloombits"
	fi
