...
#    __checkOutBranch NoSeekCompaction
	...
	if [ "$requestdistribution" = "zipfian" ]; then
	    echo "zipfian"
	    __runLSM bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" /home/ming/experiment/hlsm_"$DISK"_read/halfskipratio2_"$requestdistribution""$zipfianconst"_sizeRatio"$sizeRatio"_30WRead "$level"  "$lsmtype" "$bloombits"
	else
	    echo "$requestdistribution"
	    __runLSM bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" /home/ming/experiment/hlsm_"$DISK"_read/halfskipratio2_"$requestdistribution"_sizeRatio"$sizeRatio"_30WRead "$level"  "$lsmtype" "$bloombits"
	fi
