...
#    __checkOutBranch NoSeekCompaction
	...
	if [ "$requestdistribution" = "zipfian" ]; then
	    echo "zipfian"
	    __runLSM l01_insert_bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" /home/ming/experiment/hlsm_"$DISK"_read/skipratio2_"$requestdistribution""$zipfianconst"_sizeRatio"$sizeRatio" "$level"  "$lsmtype" "$bloombits"
	else
	    echo "$requestdistribution"
	    __runLSM l01_insert_bloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" /home/ming/experiment/hlsm_"$DISK"_read/skipratio2_"$requestdistribution"_sizeRatio"$sizeRatio" "$level"  "$lsmtype" "$bloombits"
	fi
