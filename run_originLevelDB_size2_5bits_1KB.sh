...
do
	...
	do
		...
		if [ "$requestdistribution" = "zipfian" ]; then
		    ...
		    dirname=/home/ming/experiment/lsm_"$DISK"_read_zipfian"$zipfianconst"/experiment"$experiment_time"_"$value_size"/bloombits"$bloombits"level"$level"/NoModopen_files_"$maxOpenfiles"_allfound_30WRead_directIO"$directIOFlag"_blockCacheSize"$blockCacheSize"MB_sizeRatio"$sizeRatio"
		else
		    echo "$requestdistribution"
		    dirname=/home/ming/experiment/lsm_"$DISK"_read_"$requestdistribution"/experiment"$experiment_time"_"$value_size"/bloombits"$bloombits"level"$level"/NoModopen_files_"$maxOpenfiles"_allfound_30WRead_directIO"$directIOFlag"_blockCacheSize"$blockCacheSize"MB_sizeRatio"$sizeRatio"
		fi
