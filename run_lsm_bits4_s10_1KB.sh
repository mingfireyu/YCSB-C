...
#    __checkOutBranch NoSeekCompaction
	...
	if [ "$requestdistribution" = "zipfian" ]; then
	    ...
	    do
		__runLSM l04_zipbloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" /home/ming/experiment/hlsm_"$DISK"_read/skipratio2_"$requestdistribution""$zipfianconst"_sizeRatio"$sizeRatio"_100WRead_notFound_blockCache"$blockCacheSize"MB "$level"  "$lsmtype" "$bloombits"
	    done
	else
	    echo "$requestdistribution"
	    __runLSM l04_zipbloombits"$bloombits"_level"$level"_lsmtype_"$lsmtype" /home/ming/experiment/hlsm_"$DISK"_read/skipratio2_"$requestdistribution"_sizeRatio"$sizeRatio"_100WRead_notFound_blockCache"$blockCacheSize"MB "$level"  "$lsmtype" "$bloombits"
	fi
        done
    done
done
